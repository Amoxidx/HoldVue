...
const messages = {
  ...
  en: {
    ...
    'settings.eyebrow': 'LOCAL SETTINGS', 'settings.title': 'Settings', 'settings.currency': 'Currency', 'settings.locale': 'Language', 'settings.theme': 'Theme', 'settings.automation': 'Local automation', 'settings.scheduler': 'Minute refresh', 'settings.spam': 'Spam filter enabled', 'settings.hiddenSpam': 'Show hidden spam assets', 'settings.providers': 'Enable providers', 'settings.providerConfig': 'Provider configuration', 'settings.etherscanKey': 'Free Etherscan key (encrypted)', 'settings.fmpKey': 'FMP key for automatic stock/ETF prices (encrypted)', 'settings.keyPlaceholder': 'Enter only to save', 'settings.keyConfigured': 'Etherscan key configured', 'settings.keyNotConfigured': 'No Etherscan key configured', 'settings.fmpConfigured': 'FMP key configured · automatic stock/ETF prices active', 'settings.fmpNotConfigured': 'Local instrument catalog active · automatic prices require FMP', 'settings.etherscanHelp': 'A free key enables native EVM balances and bounded token discovery; limits remain visible.', 'settings.etherscanLink': 'Request an Etherscan key', 'settings.fmpHelp': 'The local instrument catalog works without a key. An FMP key is required for automatic stock/ETF prices and expanded search. Provider limits remain visible.', 'settings.fmpLink': 'FMP documentation', 'settings.solanaEndpoint': 'Solana RPC override (HTTPS)', 'settings.rpcOverrides': 'EVM RPC overrides', 'settings.rpcHelp': 'Optional credential-free HTTPS endpoint per chain.', 'button.saveKey': 'Save key', 'button.deleteKey': 'Remove key', 'provider.evm': 'EVM JSON-RPC', 'provider.bitcoin': 'Bitcoin · mempool.space', 'provider.solana': 'Solana JSON-RPC', 'provider.cardano': 'Cardano · Koios', 'settings.wallets': 'Wallet management',
...
export function createRendererController(documentRef: RendererDocument, api: HoldVueApi): RendererController {
  ...
  const openPriceSettings = (): void => {
    focusIntent = { kind: 'settings' };
    setDialogOpen(settingsDialog, true);
    query('[data-fmp-key]')?.focus?.();
  };
  ...
  const holdingCards = (state: PortfolioState): string => {
    ...
    return holdings.map(holding => {
      ...
      const quotedPrice = quote ? formatMoney(state.settings.currency === 'EUR' ? quote.priceEurScaled : quote.priceUsdScaled, state.settings.currency) : null;
      const hasQuote = quotedPrice !== null && quotedPrice !== '—';
      const effectiveStatus = status ?? (!hasQuote && !fmpIsConfigured(state) ? { assetId: `instrument:${instrument.id}`, providerId: 'fmp.market', status: 'unpriced' as const, errorCode: 'unconfigured', lastGoodFetchedAt: null } : undefined);
      ...
      const setup = !hasQuote && effectiveStatus?.errorCode === 'unconfigured' ? `<button type="button" class="secondary holding-price-action" data-price-source-setup aria-label="${escapeHtml(localized('portfolio.setupPriceSource'))}">${escapeHtml(localized('portfolio.setupPriceSource'))}</button>` : '';
  ...
  const chooseInstrument = (candidate: InstrumentCandidate): void => {
    ...
    const hint = candidate.providerId === 'holdvue.catalog'
      ? (fmpIsConfigured(currentState!) ? localized('holdings.localPriceActiveHint') : localized('holdings.localPriceHint'))
      : (fmpIsConfigured(currentState!) ? localized('settings.fmpConfigured') : localized('holdings.fmpPriceHint'));
  ...
  const openHolding = (mode: 'add' | 'edit', id?: string): void => {
    ...
    if (instrument) {
      ...
      const hint = instrument.providerId === 'holdvue.catalog'
        ? (fmpIsConfigured(currentState!) ? localized('holdings.localPriceActiveHint') : localized('holdings.localPriceHint'))
        : (fmpIsConfigured(currentState!) ? localized('settings.fmpConfigured') : localized('holdings.fmpPriceHint'));
      setText(query('[data-instrument-price-hint]'), hint);
  ...
  const submitHolding = async (event: Event): Promise<void> => {
    ...
    if (await applyMutation(result)) { setDialogOpen(holdingDialog, false); editingHoldingId = null; selectedInstrument = null; setText(query('[data-instrument-price-hint]'), ''); holdingRestore(); }
  };
