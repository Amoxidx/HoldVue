...
const messages = {
  de: {
    ...
    'hero.eyebrow': 'LOCAL-FIRST PORTFOLIO', 'hero.title': 'Dein Portfolio bleibt lokal.', 'hero.lede': 'Öffentliche Wallet-Adressen werden nur lokal gespeichert. Es werden niemals Seeds oder private Schlüssel abgefragt.',
  ...
  en: {
    ...
    'hero.eyebrow': 'LOCAL-FIRST PORTFOLIO', 'hero.title': 'Your portfolio stays local.', 'hero.lede': 'Public wallet addresses are stored locally only. Seeds and private keys are never requested.',
...
export function createRendererController(documentRef: RendererDocument, api: HoldVueApi): RendererController {
  ...
  const renderSuggestions = (candidates: readonly InstrumentCandidate[]): void => {
    const list = query('[data-instrument-suggestions]');
    if (!list) return;
    if (candidates.length === 0) { hideSuggestions(); setInstrumentStatus(localized('holdings.empty')); return; }
    list.innerHTML = candidates.map((candidate, index) => `<li id="instrument-option-${index}" role="option" data-instrument-index="${index}" aria-selected="false"><strong>${escapeHtml(candidate.name)}</strong><span>${escapeHtml(candidate.symbol)} · ${escapeHtml(candidate.exchange)} · ${escapeHtml(candidate.currency)} · ${escapeHtml(candidate.type === 'unknown' ? localized('holdings.typePending') : candidate.type.toUpperCase())}</span></li>`).join('');
    list.hidden = false;
  ...
  const assetRow = (asset: PortfolioAssetView, currency: 'EUR' | 'USD'): string => {
    ...
    return `<article class="asset-row${expanded ? ' is-expanded' : ''}" data-asset-id="${escapeHtml(asset.assetId)}"><div class="asset-main"><button type="button" class="asset-disclosure" data-asset-disclosure="${escapeHtml(asset.assetId)}" aria-expanded="${expanded ? 'true' : 'false'}" aria-controls="${escapeHtml(chartId)}"><span class="disclosure-chevron" aria-hidden="true">${expanded ? '⌄' : '›'}</span><span><strong>${escapeHtml(asset.name)}</strong><small>${escapeHtml(asset.symbol)} · ${escapeHtml(asset.source.replace('Multiple accounts', localized('portfolio.multipleAccounts')))}</small></span></button></div><div class="asset-meta"><span class="asset-price">${escapeHtml(formatMoney(asset.priceScaled, currency))}</span><span class="asset-change${asset.dayChangeScaled?.startsWith('-') ? ' negative' : ''}">${escapeHtml(change)}</span><button type="button" class="asset-value-button" data-asset-toggle="${escapeHtml(asset.assetId)}" aria-label="${escapeHtml(toggleLabel)}">${escapeHtml(primary)}</button>${hideButton}</div>${expandedPanel}</article>`;
  };
  const renderPortfolio = (state: PortfolioState): void => {
    ...
    if (portfolioDay !== count) setText(portfolioDay, day);
