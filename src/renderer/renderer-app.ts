...
const messages = {
  de: {
    ...
    'wallet.eyebrow': 'WALLETS / WALLET-QUELLEN', 'wallet.title': 'Wallets', 'wallet.copy': 'Verwalte öffentliche Adressen und Extended Public Keys. Synchronisierung bleibt providerabhängig; keine NFTs.', 'wallet.add': 'Wallet hinzufügen', 'wallet.empty': 'Noch keine Wallets verbunden.', 'wallet.etherscanTitle': 'ERC-20: automatische On-chain-Erkennung', 'wallet.etherscanHelp': 'CoinGecko-gelistete Verträge werden ohne Key direkt auf der jeweiligen Chain geprüft. Etherscan ist optional für zusätzliche Indexer-Abdeckung; Base ist dort nicht im kostenlosen Tarif enthalten.', 'wallet.etherscanAction': 'Provider ansehen',
  ...
  en: {
    ...
    'wallet.eyebrow': 'WALLETS / WALLET SOURCES', 'wallet.title': 'Wallets', 'wallet.copy': 'Manage public addresses and extended public keys. Synchronization depends on providers; no NFTs.', 'wallet.add': 'Add wallet', 'wallet.empty': 'No wallets connected yet.', 'wallet.etherscanTitle': 'ERC-20: automatic on-chain discovery', 'wallet.etherscanHelp': 'CoinGecko-listed contracts are checked directly on each chain without a key. Etherscan is optional for additional indexer coverage; Base is not included in its free tier.', 'wallet.etherscanAction': 'View providers',
    ...
    'portfolio.aria': 'Portfolio status', 'portfolio.eyebrow': 'PORTFOLIO', 'portfolio.title': 'Portfolio', 'portfolio.copy': 'Values and prices are built locally over time.', 'portfolio.assets': 'Assets', 'portfolio.valued': 'valued', 'portfolio.unpriced': 'Price not yet available', 'portfolio.priceUnconfigured': 'No price source configured', 'portfolio.priceMissing': 'No quote is available for this instrument yet', 'portfolio.pricePartial': 'Quote is only partially available', 'portfolio.priceRateLimited': 'Price source rate limit reached', 'portfolio.priceError': 'Price source is unavailable', 'portfolio.priceStale': 'Using the last known quote', 'portfolio.priceSource': 'Price source', 'portfolio.setupPriceSource': 'Set up price source', 'portfolio.sort': 'Sort by', 'portfolio.sortSize': 'Size', 'portfolio.sortGainers': 'Gainers', 'portfolio.sortLosers': 'Losers', 'portfolio.hidden': 'Hidden assets', 'portfolio.hide': 'Hide asset', 'portfolio.restore': 'Restore', 'portfolio.showValue': 'Show value', 'portfolio.showQuantity': 'Show quantity', 'portfolio.account': 'Account', 'portfolio.balance': 'Balance', 'portfolio.chartEmpty': 'No history yet. The local series starts building now.', 'portfolio.chartTooShort': 'One point is available; more local data will follow.', 'portfolio.rangeEmpty': 'No data exists for this range yet.', 'portfolio.empty': 'No held assets yet.', 'portfolio.emptyTitle': 'Add assets', 'portfolio.emptyHelp': 'Connect a public wallet or add stocks and ETFs. All asset management now lives in Settings.', 'portfolio.emptyAction': 'Add assets', 'portfolio.multipleAccounts': 'Multiple accounts', 'portfolio.history': 'Portfolio history', 'portfolio.assetPriceHistory': 'Price history', 'portfolio.unitPrice': 'Unit price', 'portfolio.chartRange': 'Chart range',
...
type IconName = 'check' | 'chevron-right' | 'copy' | 'edit' | 'eye' | 'eye-off' | 'plus' | 'settings' | 'trash' | 'wallet';
...
export function statusToneForMessage(key: string): 'ready' | 'neutral' | 'warning' | 'busy' | 'error' {
  ...
  if (key === 'status.syncPartial' || key === 'status.syncErc20Key' || key === 'status.syncErc20Catalog' || key === 'status.syncRate') return 'warning';
...
export function createRendererController(documentRef: RendererDocument, api: HoldVueApi): RendererController {
  ...
  const syncSummary = (state: PortfolioState): MessageKey => {
    ...
    if (statuses.some(item => item.family === 'evm' && item.status === 'partial' && item.errorCode === 'unconfigured')) return 'status.syncErc20Key';
    if (statuses.some(item => item.family === 'evm' && item.status === 'partial' && (item.errorCode === 'catalog-coverage' || item.errorCode === 'catalog-rpc-partial' || item.errorCode === 'stale-catalog'))) return 'status.syncErc20Catalog';
  ...
  const setManagementControls = (state: PortfolioState): void => {
    query('[data-wallet-add-slot]')?.setAttribute?.('data-empty', String(state.wallets.length === 0));
    query('[data-holding-add-slot]')?.setAttribute?.('data-empty', String((state.holdings?.length ?? 0) === 0));
    const etherscanRelevant = state.wallets.length === 0 || state.wallets.some(wallet => wallet.family === 'evm');
    const note = query('[data-etherscan-onboarding]');
    if (note) note.hidden = !etherscanRelevant;
  };
  ...
  const renderPortfolio = (state: PortfolioState): void => {
    ...
    const hasConfiguredAssets = normalized.wallets.length > 0 || normalized.holdings.length > 0;
    ...
    for (const element of queryAll('[data-portfolio-chart], [data-portfolio-toolbar]')) element.hidden = !hasConfiguredAssets;
    if (list && list !== count) {
      const sorted = sortPortfolioAssets(model.assets, portfolioSort);
      const addAssets = `<section class="portfolio-empty-state"><span class="portfolio-empty-mark" aria-hidden="true">${iconMarkup('wallet')}</span><h3>${escapeHtml(localized('portfolio.emptyTitle'))}</h3><p>${escapeHtml(localized('portfolio.emptyHelp'))}</p><button type="button" class="primary action-button portfolio-empty-action" data-open-asset-settings>${iconMarkup('plus')}<span>${escapeHtml(localized('portfolio.emptyAction'))}</span></button></section>`;
      list.innerHTML = sorted.length === 0 ? (hasConfiguredAssets ? `<p class="holding-empty">${escapeHtml(localized('portfolio.empty'))}</p>` : addAssets) : sorted.map(asset => assetRow(asset, currency)).join('');
    }
    ...
    for (const button of queryAll('[data-open-asset-settings]')) bind(button, 'click', event => { event.preventDefault(); focusIntent = { kind: 'settings' }; setDialogOpen(settingsDialog, true); query('[data-add-wallet]')?.focus?.(); });
  ...
  const bindEvents = (): void => {
    ...
    bind(query('[data-open-settings]'), 'click', event => { event.preventDefault(); focusIntent = { kind: 'settings' }; setDialogOpen(settingsDialog, true); (query('[data-add-wallet]') ?? query('[data-setting-currency]'))?.focus?.(); });
    bind(query('[data-open-etherscan-settings]'), 'click', event => { event.preventDefault(); focusIntent = { kind: 'settings' }; setDialogOpen(settingsDialog, true); query('[data-etherscan-key]')?.focus?.(); });
