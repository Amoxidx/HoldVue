...
const messages = {
  de: {
    ...
    'wallet.eyebrow': 'WALLETS / WALLET-QUELLEN', 'wallet.title': 'Wallets', 'wallet.copy': 'Verwalte öffentliche Adressen und Extended Public Keys. Synchronisierung bleibt providerabhängig; keine NFTs.', 'wallet.add': 'Wallet hinzufügen', 'wallet.empty': 'Noch keine Wallets verbunden.', 'wallet.etherscanTitle': 'ERC-20: kostenloser Etherscan-Key', 'wallet.etherscanHelp': 'Native EVM-Bestände funktionieren ohne Key. Für ERC-20-Token ist ein kostenloser Etherscan-Key erforderlich; eine Preisquelle ersetzt keine Bestandserkennung.', 'wallet.etherscanAction': 'In Einstellungen',
  ...
  en: {
    ...
    'wallet.eyebrow': 'WALLETS / WALLET SOURCES', 'wallet.title': 'Wallets', 'wallet.copy': 'Manage public addresses and extended public keys. Synchronization depends on providers; no NFTs.', 'wallet.add': 'Add wallet', 'wallet.empty': 'No wallets connected yet.', 'wallet.etherscanTitle': 'ERC-20: free Etherscan key', 'wallet.etherscanHelp': 'Native EVM balances work without a key. ERC-20 tokens require a free Etherscan key; a price source cannot replace balance discovery.', 'wallet.etherscanAction': 'Open Settings',
...
export function statusToneForMessage(key: string): 'ready' | 'neutral' | 'warning' | 'busy' | 'error' {
  ...
  if (key === 'status.syncPartial' || key === 'status.syncErc20Key' || key === 'status.syncRate') return 'warning';
...
export function createRendererController(documentRef: RendererDocument, api: HoldVueApi): RendererController {
  ...
  const setManagementControls = (state: PortfolioState): void => {
    ...
    const etherscanConfigured = (state.settings.providerRefs ?? []).some(reference => reference.providerId === 'evm.erc20' && reference.keyId !== null && reference.enabled);
    ...
    if (note) note.hidden = etherscanConfigured || !etherscanRelevant;
  };
