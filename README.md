...
- implemented/configured with an optional encrypted key: FMP stock/ETF symbol and name search. Search suggestions are persisted only after an official FMP profile verifies symbol, exchange, active status, and `isEtf`/`isFund` classification. Search never supplies prices;
...
FMP search uses the official [stable search-symbol/search-name API](https://site.financialmodelingprep.com/developer/docs/stable) with the `apikey` header and the official [Company Profile classification](https://site.financialmodelingprep.com/developer/docs/changelog). A key and a provider tier that permits these calls are required; unauthorized, rate-limited, malformed, or unclassified responses are shown as provider failures and never create a holding.
